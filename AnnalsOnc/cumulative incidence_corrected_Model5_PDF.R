...
setwd(paste0(root, '/Revised/AnnalsOnc'))
...
final_model = read_xlsx('figure_2_all_new_13.xlsx', sheet = "Model_13")
...
p3= ggcuminc(m3, outcome = c("1"),linetype_aes = TRUE) +
  ...
  custom_theme
p3
ggsave("cif_ighg_sjlife.eps", plot=p3, dpi=300, width=7.5, height=7)
# CCSS
# df3 = subset(df, source=="ccss")
# prop.table(table(df3$gp, df3$eve_Cardiomyopathy), 1)
...
p4 = ggcuminc(m4, outcome = c("1"),linetype_aes = TRUE) +
  ...
  custom_theme
p4
ggsave("cif_ighg_ccss.eps", plot=p4, dpi=300, width=7.5, height=7)
# CIF according to Model 13-based risk groups
# SJLIFE
# df2 = subset(df, source=="stjd")
# prop.table(table(df2$risk_groups_pred, df2$eve_Cardiomyopathy), 1)
m5=tidycmprsk::cuminc(Surv(years, as.factor(ci_even)) ~ risk_groups_pred_n, df2)
p5 = ggcuminc(m5, outcome = c("1"),linetype_aes = TRUE) +
  xlab("Time since baseline, years") +  ylab("Cumulative incidence")+
  coord_cartesian(xlim = c(0,10), ylim = c(0, 0.3))+
  add_risktable(theme = list(theme_risktable_default(), scale_y_discrete(label=c('Low', 'Moderate', 'High')))) +
  scale_ggsurvfit()+
  ...
  custom_theme
p5
ggsave("cif_model_sjlife.png", plot=p5, dpi=300, width=7.5, height=7)
# CCSS
# df3 = subset(df, source=="ccss")
m6=tidycmprsk::cuminc(Surv(years, as.factor(ci_even)) ~ risk_groups_pred_n, df3)
p6 = ggcuminc(m6, outcome = c("1"),linetype_aes = TRUE) +
  xlab("Time since baseline, years") +  ylab("Cumulative incidence")+
  coord_cartesian(xlim = c(0,10), ylim = c(0, 0.1))+
  add_risktable(theme = list(theme_risktable_default(), scale_y_discrete(label=c('Low', 'Moderate', 'High')))) +
  scale_ggsurvfit()+
  ...
  custom_theme 
p6
ggsave("cif_model_ccss.png", plot=p6, dpi=300, width=7.5, height=7)
