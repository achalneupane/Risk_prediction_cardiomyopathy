...
data = read.table('AUC_final_model.txt', header = TRUE, sep = "\t")
...
## Plot
pdf('AUC_overall.pdf', width = 9.5, height = 6.5, bg = "white")
data$Models = as.factor(data$Models)
...
stat_test$group1 = rep(c(NA, as.character(stat_test$Models[c(1:2, 3, 4)])), 2)
stat_test$group2 = rep(c(NA, as.character(stat_test$Models[c(2:5)])), 2)
